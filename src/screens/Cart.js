...
export default function Cart() {
  ...
  const openBottomSheet = async () => {
    ...
    try {
      const data = {
        amount: Number(getTotal().toFixed(2).toString()) * 100,
      };
     const token =await AsyncStorage.getItem('userToken')
      const response = await axios.post(
        `${APIBASEURL}/api/payment/create`,
        data,
        {
          headers: {
      ...
      const {customer, ephemeralKey, paymentIntent_id, paymentIntent} = response.data;

      const {error} = await initPaymentSheet({
        customerId: customer,
        customerEphemeralKeySecret: ephemeralKey,
        paymentIntentClientSecret: paymentIntent,
        merchantDisplayName: 'Glam Parlor, Inc.',
        returnURL: 'https://glamparlor.com/',
        allowsDelayedPaymentMethods: false,
      });

      if (!error) {
        openPaymentSheet(paymentIntent_id);
      } else {
        setLoading(false);
      }
    } catch (error) {
      console.log(error.response.data, 'err in payment');
    } finally {
      setLoading(false);
    }
    // }
  ...
  const openPaymentSheet = async () => {
    ...
    console.log('payemtn enter ', error);
    if (error) {
      Alert.alert(`Error code: ${error.code}`, error.message);
      setLoading(false);
    } else {
      console.log('payemtn successful');
      handleCheckout()
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
          <CartButton loading={loading} title="Checkout" onPress={openBottomSheet} />
